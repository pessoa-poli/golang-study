...
// Example of using time.After and Select Statement to create Timeouts
func main() {
	// This is very much like having a service
	c := fanIn(boring("Joe"), boring("Ann"))
	for { // Because of the synchonization nature of the channels, the messages here will be print in order. If ann is ready to send a message but joe has not sent his yet, ann will have to wait.
		select {
		case s := <-c:
			fmt.Println(s)
